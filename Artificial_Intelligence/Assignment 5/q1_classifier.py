...
def main():
    '''
        Usage: q1_classifier.py -p <pvalue> -f1 <train_dataset> -f2 <test_dataset> -o <output_file> -t <decision_tree>
        ...
        This function creates a linked set of nodes that process a list of features and return a 'will click',
    ...
    # translate decision tree into grading tree
    outputTree = DecisionTreetoTestingTree(decisionTree)
    # save tree to output
    ...
    for i in range(len(testData)):
        ...
        # FOR SELF EVALUATION print results
        # print "tree count: ", treecount(outputTree)
...
def ID3Ply(trainingData, labelList, testPValue, skipList):
    ...
    for i in range(len(featureList)):
        ...
        # for non-normalized data sets, set ddof = len(feature.getAttrs()) - 1
        # in this case the data has been normalized in 5 discrete feature values so ddof = 5 - 1
        chi2, curPValue = stats.chisquare(actual, f_exp=expected, ddof=4)
...
def DecisionTreetoTestingTree(node):
    ...
    for tuple in node.checks:
        ...
        children[featureVal] = DecisionTreetoTestingTree(tuple[1])
